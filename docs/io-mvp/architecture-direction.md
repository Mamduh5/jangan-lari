...
- Treat the normal stage timer as a boss-arrival clock, not a victory countdown. Runs move from pre-boss farming/survival into a boss phase at the configured 15:00 spawn time, and victory belongs to boss defeat while player death remains defeat.
- Keep boss phase focused: suppress normal waves and scheduled run events, clear non-boss enemies and neutral shape clutter on transition, and expose stage phase/boss state in debug snapshots.
- Keep A9 boss durability as a first-pass data tuning layer only. Full boss phases, unique boss skills, miniboss skill changes, and broad event/miniboss/boss rebalance belong to A10 or later balance sprints.
- Keep boss/miniboss encounter logic data-driven and observable: phase thresholds, shockwave timing/radius/damage, miniboss volley shape, and event multipliers should live in constants or shared contract helpers rather than scene-local magic numbers.
- Keep the A10 boss model intentionally small: exactly two Behemoth phases, one contract-backed boss skill pass, one additional miniboss skill, and focused event stat multipliers. Final fastest-kill tuning, boss summons/minions, and broader encounter rebalance remain later work.
- Keep boss balance tunable through constants/data. The A11 target fastest-kill value is a manual feel target for APK validation, while boss HP, phase pressure, and summon caps remain observable through debug snapshots.
- Keep boss-owned summons narrow and explicit: they are allowed only during boss phase, must be capped, must use safe spawn placement, must be tagged for debug/bot visibility, and must be cleared when boss victory resolves. Normal waves and run events stay suppressed during boss phase.
